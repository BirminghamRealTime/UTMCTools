...
def allcurrentsensordata(url="http://butc.opendata.onl/UTMC flow.xml",bbox=(-2.1691, 52.3088, -1.5930, 52.6801),update=5):
	"""
	...
	if usecache=='no':
		...
		for flow in root.iter('Flow'):
			...
			timestamp = datetime.strptime(flow.find('Date').text, '%Y-%m-%d %H:%M:%S')
			if timestamp > x:
				scnno = flow.find('SCN').text
				z += 1
				easting = float(flow.find('Easting').text)
				northing = float(flow.find('Northing').text)
				lat, lon = 0, 0
				# convert easting and northing to WGS84 if data available, otherwise get it from openstreetmap
				if easting <> 0:
					lat, lon = OSGB36toWGS84(float(flow.find('Easting').text), float(flow.find('Northing').text))
				else:
					notfound=True
				...
				if lat <> 0:
					if scnno in bearinglookup:
						...
						if flow.find('Description') is not None:
							sensorreturn[scnno]={"Location":flow.find('Description').text, "Traffic flow":int(flow.find('Value').find('Level').text),"Latitude": lat, "Longitude":lon, "Sensor bearing":int(bearinglookup[scnno]["Sensor bearing"]), "Compass direction":genbearing, "OSM Way ID":bearinglookup[scnno]["OSM Way ID"], "Lanes covered":bearinglookup[scnno]["Lanes covered"],"Lane numbers":bearinglookup[scnno]["Lane numbers"],"Total lanes":bearinglookup[scnno]["Total lanes"]}
						else:
							sensorreturn[scnno]={"Location":None, "Traffic flow":int(flow.find('Value').find('Level').text),"Latitude": lat, "Longitude":lon, "Sensor bearing":int(bearinglookup[scnno]["Sensor bearing"]), "Compass direction":genbearing, "OSM Way ID":bearinglookup[scnno]["OSM Way ID"], "Lanes covered":bearinglookup[scnno]["Lanes covered"],"Lane numbers":bearinglookup[scnno]["Lane numbers"],"Total lanes":bearinglookup[scnno]["Total lanes"]}
					else:
						if flow.find('Description') is not None:
							sensorreturn[scnno]= {"Location":flow.find('Description').text, "Traffic flow":int(flow.find('Value').find('Level').text), "Latitude":lat, "Longitude":lon,"Sensor bearing": -1,"Compass direction":"Unknown","OSM Way ID": -1,"Lanes covered": -1,"Lane numbers":[-1],"Total lanes":-1}
						else:
							sensorreturn[scnno]= {"Location":None,"Traffic flow":int(flow.find('Value').find('Level').text), "Latitude":lat, "Longitude":lon,"Sensor bearing": -1,"Compass direction":"Unknown","OSM Way ID": -1,"Lanes covered": -1,"Lane numbers":[-1],"Total lanes":-1}
	...
	with open('unfound.csv','wb')as handle:
...
if __name__=="__main__":
	x= allcurrentsensordata()

	with open('sensors.csv', 'wb') as csvfile:
		spamwriter = csv.writer(csvfile)
		spamwriter.writerow(['Sensor Ref','Location', 'Latitude', 'Longitude', 'Sensor Bearing','Compass Direction', 'OSM Way ID'])
		for n in x:
			if x[n]["Location"]==None:
				g=geocoder.osm([x[n]["Latitude"],x[n]["Longitude"]],method='reverse')
				time.sleep(1)
				if g.street<>None:
					x[n]["Location"]=g.street+" *"
			print n, x[n]["Location"],x[n]["Latitude"],x[n]["Longitude"],x[n]["Sensor bearing"],x[n]["Compass direction"],x[n]["OSM Way ID"]
			spamwriter.writerow([n, x[n]["Location"],x[n]["Latitude"],x[n]["Longitude"],x[n]["Sensor bearing"],x[n]["Compass direction"],x[n]["OSM Way ID"]])

	#x = findsensordirection(update=0)
...
# N38112B': {'Traffic flow': 45, 'Total lanes': 1, 'Location': '2017-01-20 11:37:45', 'Sensor bearing': 0, 'Latitude': '52.4553291', 'Compass direction': 'N', 'Lane numbers': [1], 'Lanes covered': 1, 'Longitude': '-1.9046504', 'OSM Way ID': 51196457
